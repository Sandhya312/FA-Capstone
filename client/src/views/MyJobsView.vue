<template>
    <div class="posted-jobs">
        <h1>Posted job page</h1>
        <div class="job-container ">
        <div class="job-card" v-for="job in postJobs" :key="job._id">
          <div class="rectangle">
            <h4>{{ job.title }}</h4>
        ...
        <form :action="`/jobs/${job._id}/appliedUsers`" method="get">
          <button class="view-application" type="submit">  <p> View Applications</p></button>
        </form>
          </div>
        </div>
    ...
    methods:{
        async getPostJobs(){
          try{
            ...
            const response = await axios.get(`https://fa-capstone.onrender.com/user/${id}/myjobs`)
            this.postJobs = response.data.jobs;
          }
          catch(err){
            console.log(err);
          }
